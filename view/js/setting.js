...
async function first_read(){
    ...
    console.log(importStartTime,importEndTime);
...
function getCoordinateInCanvas(canvas,sparse){
  ...
  function renderFigure3dLayout(data1,data2,data3,layer){
    ...
    var canvas3dBox = document.getElementById('preview-3d-image');
    canvas3dBox.appendChild(canvas3d);

    var canvas3d = document.getElementById('layout-3d');
    canvas3d.style.width = "633"
    canvas3d.style.height = "385"
  ...
  function highlight3dObjectInCanvas(layer){
    ...
    var parentElement = document.getElementById('preview-3d-image');
  ...
  function changeLayer(){
    var layer = getSelectedValue('height-select-box');
    if (layer != -1){
